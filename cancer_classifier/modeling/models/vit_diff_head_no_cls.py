...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_hiddens, num_heads, dropout, bias=False, **kwargs):
        ...
        def output_features_size(i):
            return (num_hiddens // num_heads) + (num_hiddens % num_heads if i == num_heads - 1 else 0)

        self.W_q = nn.ModuleList([nn.LazyLinear(output_features_size(i), bias=bias) for i in range(num_heads)])
        self.W_k = nn.ModuleList([nn.LazyLinear(output_features_size(i), bias=bias) for i in range(num_heads)])
        self.W_v = nn.ModuleList([nn.LazyLinear(output_features_size(i), bias=bias) for i in range(num_heads)])
...
class Vit_diff_head_no_cls_model(nn.Module):
    """Vision Transformer."""
